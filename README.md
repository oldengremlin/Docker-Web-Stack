...
`apply` перед будь-якою дією показує, які контейнери буде створено,
перестворено й прибрано — він порівнює хеші конфігурації compose, тобто
рівно те, за чим сам docker вирішує, чи чіпати контейнер. Переходити
можна й по одному домену: `render --force <домен>` і потім `apply`.
...
потрапляє в бекап** — зберігайте паролі окремо; без них дампи відновити
можна, а контейнери підняти ні.
