...
| [`up` / `down` / `restart`](#up--down--restart-домен--живі-контейнери) | Керує живими контейнерами, не чіпаючи склад стека. З доменом — лише його контейнери, без домену — весь стек. | так |
...
### `up` / `down` / `restart [<домен>]` — живі контейнери

**Що.** Піднімає, зупиняє і перезапускає контейнери — і більше нічого.
Нічого не генерує, нічого не перезаписує, реєстру не читає далі за тип
домену. Змінити **склад** стека — це [`apply`](#apply---force--застосувати-до-працюючого-стека).

```bash
dcpmgmt restart example.com     # БД і php-fpm цього домену
dcpmgmt down                    # весь стек
```

**Як.** З іменем домену бере лише його контейнери:

| Тип | Що чіпає |
|---|---|
| `wordpress` | `db-<sid>` і `wordpress-<sid>` разом; порядок гарантує `depends_on` |
| `proxy` | його власний сервіс із `docker-compose.domains/<домен>.yml` |
| `redirect` | нічого — контейнерів у нього немає, він живе рядками в nginx |

Без імені домену працює на весь стек: `up` — `compose up -d`, `restart` —
`compose restart`, `down` — `compose down --remove-orphans`, тобто
контейнери разом із мережею проєкту. Дані при цьому не страждають: усі
volume'и названі й bind-backed, каталоги на хості лишаються як були.

Після `up` і `restart` виконується `nginx -s reload`.

**Навіщо reload.** nginx резолвить імена з `fastcgi_pass` і `proxy_pass`
один раз — на старті воркерів — і тримає адресу до наступного
перезавантаження конфігурації. Перестворений контейнер отримує іншу
адресу в мережі docker, і без reload nginx уперто стукає в стару: домен
щойно підняли, а він віддає 502. Після `down` reload навпаки не робиться
— зупинений upstream не резолвиться, і перевірка конфігу просто не
пройшла б.

**Навіщо окремо від `apply`.** `apply` відповідає на питання «що має
бути»: перегенеровує конфігурацію і перестворює контейнери, склад яких
розійшовся з описом. `restart` відповідає на «перезапусти оце» — коли
конфігурація ні до чого, а сервіс просто треба смикнути.
...
з неіснуючим доменом мовчки нічого не зробить.

Імена беруться з `dcpmgmt list --names` тією ж копією інструмента, яку
ви набрали в рядку, тож підказка відповідає саме цій інсталяції. Поза
каталогом інсталяції нічого не підкаже — і не зашкодить. Працює і через
`sudo`, і для `./dcpmgmt`.

`deploy.sh` кладе цей файл в інсталяцію разом із рештою `tools/`, але в
системі нічого не чіпає: встановлення — свідома дія.

## Оновлення інсталяції

Інструмент і шаблони живуть у клоні репозиторію, а працює все в каталозі
інсталяції — там, де `dcp.conf`, `domains/` і `secrets/`. Щоб оновити
...
compose-файли лишаються недоторканими. Наприкінці він виконує
`dcpmgmt version` уже в цільовому каталозі, тож ви бачите версію тієї
копії, яка там опинилася.

**Чому не `cp` руками.** Частин три — інструмент, шаблони, `tools/` — і
забути одну нічого не коштує. Найгірше, що `dcpmgmt version` показує
версію лише самого скрипта: новий код поруч зі старими шаблонами
виглядає як справний стек, а генерує при цьому вчорашні конфіги.

Запускати треба саме з клону: в інсталяції може лежати як завгодно стара
версія, яка про цю команду ще не знає.
