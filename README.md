...
nginx/custom/<домен>.conf     ваші правки nginx, не чіпаються ніколи
dcpmgmt                       інструмент
...
/var/sites/<домен>/php/*.ini  налаштування PHP (монтуються в conf.d)
...
* **`/var/sites/<домен>/php/*.ini`** — ваші. Реєстр про них не знає,
  інструмент їх не створює і не змінює, а лише монтує в
  `/usr/local/etc/php/conf.d/` контейнера. Так налаштування PHP
  (`upload_max_filesize`, `post_max_size`, `memory_limit`) живуть поруч
  із сайтом і переживають `render --force`. Деталі — у розділі
  «Налаштування PHP».
